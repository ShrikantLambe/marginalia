...
/**
 * Web mode of the Find page: guardrailed web search through the user's trusted
 * sources, with capture. This is the former Discover page minus its own header
 * and input — the Find shell owns those. Searches run when `runSignal`
 * increments (Enter in the shared input, escalation from Library, or a deep
 * link), or when a saved/recent/try chip is clicked.
 */
export function DiscoverPanel({
  ...
  // Restore last-used scope; load saved + recent searches
  useEffect(() => {
    try {
  ...
  // Run when the shell signals (Enter / escalation / deep link)
  useEffect(() => {
    if (runSignal > 0) void runSearch(query, scope);
  ...
  if (sources.length === 0) {
    return (
      <div className="border border-rule p-6">
        <p className="font-serif text-[15px] mb-2">Web search only looks at sites and writers you trust.</p>
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-2">
        ...
        {query.trim() && (
          <button onClick={saveSearch} className="ml-auto font-mono text-[10px] tracking-[0.12em] uppercase text-muted hover:text-ink transition-colors">
            Save search
          </button>
        )}
      </div>
